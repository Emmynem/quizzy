...
import { default_status, check_length_TEXT, return_default_value, check_pro_expiry, true_status } from '../config/config.js';
    ...
    forAdding: [
        check('question_unique_id', "Question Unique Id is required")
            ...
            .custom(async (question_unique_id, { req }) => {
                const answer_count = await ANSWERS.count({ where: { question_unique_id } });
        ...
        check('answer', "Indicate if option is the correct answer")
            ...
            .custom(async (answer, { req }) => {
                const question_details = await QUESTIONS.findOne({ attributes: ['multiple_answer'], where : { unique_id: req.query.question_unique_id || req.body.question_unique_id || '' } });
                const multiple_answer_status = question_details['dataValues'].multiple_answer;
                const answer_details = await ANSWERS.findOne({ attributes: ['answer'], where : { question_unique_id: req.query.question_unique_id || req.body.question_unique_id || '', answer: true_status } });
                const answer_status = answer_details['dataValues'].answer;
    ...
    forUpdatingDetails: [
        ...
        check('order', "Order is required")
            ...
            .custom(async (order, { req }) => {
                const answer_count = await ANSWERS.count({ where: { question_unique_id: req.query.question_unique_id || req.body.question_unique_id || '' } });
